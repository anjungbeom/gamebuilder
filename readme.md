<p align="center">
  <img src="thumbnail.png" alt="Drawn Frontier — 길이 없으면, 그려서 연다" width="880">
</p>

<h1 align="center">Drawn Frontier</h1>
<p align="center"><b>길이 없으면, 그려서 연다</b></p>
...
화면에 그은 획의 길이, 꺾인 각도, 감싼 넓이, 갈래의 수가 그대로 장비의 성능이 되는
절차 생성 도트 탐험 게임. 
일정 크기의 랜덤 맵을 모두 개척하는것이 목표!
...
| 플랫폼 | 데스크톱 브라우저 (Chrome · Edge · Safari · Firefox) |
| 가격 | 무료 |
| 언어 | 한국어 |
| 한 판 길이 | 10–30분 |
...
**끝없이 이어지는 개척지.**
시드와 좌표로 생성되는 청크 위에 고도·습도·온도 노이즈가 바이옴을 만든다.
...
| `M` | 확장 지도 |
| `C` | 크리처 도감 |
| `X` · `E` · `R` | 해금 후 점프 · 와이어 · 반사 방벽 |

## 3분 만에 감 잡기

1. 새 원정을 시작하고 나침반이 가리키는 가장 가까운 신호기 방향을 본다.
2. `Q`로 길고 뾰족한 획을 긋는다. 그리는 동안 수치가 실시간으로 움직인다.
