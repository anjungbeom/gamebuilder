...
import {
  TILE, biomeAt, obstacleAt, isWater, isBiomeSolid,
...
const STAT_DEFS = [
  { key: "edge", label: "쐐기력", color: "var(--edge)", tick: OBSTACLE_RULE.boulder.threshold },
  { key: "reach", label: "도달력", color: "var(--reach)", tick: OBSTACLE_RULE.thicket.threshold },
  { key: "buoy", label: "부력", color: "var(--buoy)", tick: WATER_RULE.threshold },
  { key: "grip", label: "결속력", color: "var(--grip)", tick: OBSTACLE_RULE.bramble.threshold }
];
...
let mode = "title";           // title | play | draw | dex | win
let game = null;
...
function updateCreatures(dt) {
  for (const c of creatures.values()) {
    ...
    if (c.genome.skittish > 0.3 && dist < fleeRange && dist > 0.01) {
...
function buildTool(strokes) {
  ...
  return {
    strokes: normalized,
    stats,
    name: nameTool(stats, Math.round(stats.spanPx)),
...
function useTool() {
  ...
  if (c) {
    ...
    if (have >= need) {
      ...
      game.dex.set(c.genome.species, { name: c.genome.name, biome: c.genome.biome });
      toast(first ? `새 생태 기록 — ${c.genome.name}` : `${c.genome.name} — 관찰 기록 있음`, "good");
      creatures.delete(`${c.cx},${c.cy}`);
      spendTool();
      saveNow();
    } else {
      toast(`결속력이 모자란다 — ${have.toFixed(2)} / ${need.toFixed(2)}`, "bad");
    }
    dirty = true;
    return;
  }
...
function confirmDraw() {
  ...
  saveNow();
  closeDraw();
  toast(`${tool.name} 제작 — 사용 가능 횟수 ${tool.durability}`, "good");
}
...
const barRows = STAT_DEFS.map(def => {
  const row = document.createElement("div");
  row.className = "bar-row";
  row.innerHTML =
    `<span class="lbl">${def.label}</span>` +
    `<span class="track"><i class="fill" style="background:${def.color}"></i>` +
    `<i class="tick" style="left:${(def.tick * 100).toFixed(1)}%"></i></span>` +
    `<span class="val">0</span>`;
  el.bars.appendChild(row);
  return { def, row, fill: row.querySelector(".fill"), val: row.querySelector(".val") };
});

function statsForHud() {
  if (mode === "draw") return analyzeStrokes(strokes, PAD_W, PAD_H);
...
function updateHud() {
  ...
  for (const b of barRows) {
    const v = stats ? stats[b.def.key] : 0;
    b.fill.style.width = `${(v * 100).toFixed(1)}%`;
    b.val.textContent = v.toFixed(2);
    b.row.classList.toggle("met", v >= b.def.tick);
  }

  if (mode === "draw") {
    const d = stats?.durability ?? 0;
    el.toolName.textContent = stats && d > 0 ? nameTool(stats, Math.round(stats.spanPx)) : "형질 분석 중";
...
function updateHint() {
  ...
  if (c) {
    ...
    el.hint.innerHTML = have >= need
      ? `${c.genome.name} — <b>Space</b>로 생태 기록`
      : `${c.genome.name} — 결속력 <b>${need.toFixed(2)}</b> (현재 ${have.toFixed(2)})`;
    el.hint.hidden = false;
    return;
  }

  if (!canFloat() && nearWater()) {
    el.hint.innerHTML = `깊은 물길 — 부력 <b>${WATER_RULE.threshold.toFixed(2)}</b> 이상이면 건널 수 있다`;
    el.hint.hidden = false;
    return;
  }
  el.hint.hidden = true;
...
function showTitle() {
  ...
  el.overlay.innerHTML = `
    <div>
      ...
      <p class="keys">
        <b>WASD</b> 이동 · <b>Q</b> 도구 설계 · <b>Space</b> 사용 · <b>C</b> 생태 기록<br>
        길게 그리면 도달력, 뾰족하면 쐐기력, 닫으면 부력, 여러 획이면 결속력
      </p>
    </div>`;
}
    ...
    <div>
      ...
      ${entries.length
        ? `<div class="dex">${entries.map(e =>
            `<div class="dex-entry"><div class="nm">${e.name}</div><div class="bi">${e.biome}</div></div>`
          ).join("")}</div>`
...
window.addEventListener("keydown", e => {
  if (e.code in MOVE_KEYS || ["Space", "KeyQ", "KeyC", "Escape", "Enter", "Backspace"].includes(e.code)) {
    e.preventDefault();
  }
  keys.add(e.code);
  if (e.repeat) return;
...
function movePlayer(dt) {
  ...
  if (!blockedAt(nx, game.py)) game.px = nx;
  if (!blockedAt(game.px, ny)) game.py = ny;
...
function render() {
  ...
  for (const d of drawables) {
    if (d.kind === "creature") {
      ...
      drawPlayer(ctx, sx, sy, game.facing, game.walking, game.time, isAfloat());
      drawHeldTool(ctx, game.tool, sx, sy, game.facing, game.swing);
    }
  }
